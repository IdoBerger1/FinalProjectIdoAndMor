...
int main(int argc, char *argv[])
{
	...
	t_timer tt; // timer
	const size_t rowA = 8100;
	const size_t colA_rowB = 7900;
	const size_t colB = 6000;

	cout << "Block Size = " << I_BLOCKSIZE << comma << " ITER_NUM = " << ITER_NUM << comma << " rowA = " << rowA << comma << " colA_rowB = " << colA_rowB << comma << " colB = " << colB << endl;
	...
	//result << t_avx << comma << t_avx_1 << comma << t_mkl << comma << "" << comma << t_avx / t_mkl << comma << "" << comma << t_avx_1 / t_mkl << endl;
